...
export const StakesModal: FC<StakesModalSettings> = ({
  ...
  async function fetchUserBalance() {
    const contract = new ethers.Contract(chainConfig.contracts.ARCHI, erc20Abi, appRpcProvider);
  ...
  async function handleStake() {
    ...
    try {
      ...
      log(`${amountWei} ARCHI successfully supplied.`);
    } catch (e) {
      trackError(e, tx);
      setModal(null);
  ...
  return (
    <Modal>
      ...
      <div className="flex flex-col space-y-2 mb-6">
        ...
        >
          Balance: <span className="text-purple-600">{balance.formatted.toString()} ARCHI</span>
        </div>
      ...
      <p className="text-lg text-center mb-6">
        You are staking: {getDisplayAmount(walletStakesSum[selectedStake.sec.toString()] || 0)} ARCHI
        tokens
      </p>
      {needApproval && (
        <OpacityBox className="items-center text-lg bg-none border border-gray-400 mb-10">
          <ul>
            ...
            <li className="pl-4">1. Approve ARCHI for Stake</li>
            <li className="pl-4">2. Stake ARCHI tokens</li>
          </ul>
        </OpacityBox>
      )}
      <UIButton className="w-full" onClick={handleStake} disabled={wrongNetwork}>
        {wrongNetwork ? 'Wrong network' : needApproval ? 'Approve ARCHI and Stake' : 'Stake'}
      </UIButton>
    </Modal>
  );
};
