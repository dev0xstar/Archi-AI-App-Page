...
import Tiker from 'src/assets/images/IGD-tiker.svg?react';
      ...
      <OpacityBox className="flex flex-col absolute overflow-hidden left-0 top-0 w-full h-full">
        ...
        <div className="text-3xl">
          {getDisplayAmount(sub.setupPrice, {
            decimals: sub.payableToken.decimals,
          })}{' '}
        ...
        {isUpgradeable && (
          <ExecuteButton
            ...
            onClick={onPay}
            color={index === 0 ? 'default' : index === 1 ? 'orange' : 'blue'}
            label={
              sub.isPaused
                ? 'Tariff paused'
